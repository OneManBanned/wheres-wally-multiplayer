...
import { isAnimatingFill } from "./animations.js";
...
export function refreshMagnifier(image = DOM.mainPuzzle) {
    ...
    const lens = magnifier.querySelector(".lens-content");
    const rect = image.getBoundingClientRect();
    const currentX = parseFloat(magnifier.style.left) + magnifierConfig.lensSize / 2;
    const currentY = parseFloat(magnifier.style.top) + magnifierConfig.lensSize / 2;

    const isFlipped = image.dataset.flipped === "true";
    const bgX = isFlipped
        ? -( (rect.width - currentX) * magnifierConfig.zoomLevel - magnifierConfig.lensSize / 2)
        : -(currentX * magnifierConfig.zoomLevel - magnifierConfig.lensSize / 2);
    const bgY = isFlipped 
        ? -( (rect.height - currentY) * magnifierConfig.zoomLevel - magnifierConfig.lensSize / 2)
